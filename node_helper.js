...
module.exports = NodeHelper.create({
  ...
  initGoogleAssistant: function(payload) {
	...
	// console.log("text: " + message.message.text);
        if (message.message.text === 'ON_CONVERSATION_TURN_STARTED') {
          self.sendSocketNotification('ON_CONVERSATION_TURN_STARTED', null);
        } else if (message.message.text === 'ON_CONVERSATION_TURN_FINISHED') {
          self.sendSocketNotification('ON_CONVERSATION_TURN_FINISHED', null);
        } else if (message.message.text.includes('ON_RECOGNIZING_SPEECH_FINISHED')) {
          var query = message.message.text.split(":");
